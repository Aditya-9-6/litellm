...
from litellm.proxy._types import PasswordPolicy, UserAPIKeyAuth
...
_POLICY_NO_BREACH_CHECK = {"password_policy": PasswordPolicy(check_breached_passwords=False)}
...
async def test_change_password_enforces_min_length():
    ...
    assert exc_info.value.status_code == 400
    assert "at least 12 characters" in exc_info.value.detail["error"]
    prisma.db.litellm_usertable.update.assert_not_called()
...
async def test_change_password_rejects_breached_password():
    ...
    breached_password = "Password123456"
