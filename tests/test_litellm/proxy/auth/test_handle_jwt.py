...
def test_get_jwt_role_reads_a_scalar_claim_as_one_role_and_drops_non_strings():
    jwt_handler = JWTHandler()
    jwt_handler.litellm_jwtauth = LiteLLM_JWTAuth(roles_jwt_field="roles")

    assert jwt_handler.get_jwt_role({"roles": "admin"}, default_value=None) == ["admin"]
    assert jwt_handler.get_jwt_role({"roles": ["admin", 42, None]}, default_value=None) == ["admin"]
    assert jwt_handler.get_jwt_role({}, default_value=None) is None


def test_scalar_role_claim_matches_mappings_as_a_whole_role_not_per_character():
    """
    A scalar claim used to be iterated character by character: `admin` never matched an
    `admin` mapping and `platform-admins` never matched `platform-*`.
    """
    jwt_handler = JWTHandler()
    jwt_handler.litellm_jwtauth = LiteLLM_JWTAuth(
        roles_jwt_field="roles",
        role_mappings=[RoleMapping(role="admin", internal_role=LitellmUserRoles.PROXY_ADMIN)],
        jwt_litellm_role_map=[JWTLiteLLMRoleMap(jwt_role="platform-*", litellm_role=LitellmUserRoles.PROXY_ADMIN)],
    )

    assert jwt_handler.get_rbac_role({"roles": "admin"}) == LitellmUserRoles.PROXY_ADMIN
    assert jwt_handler.map_jwt_role_to_litellm_role({"roles": "platform-admins"}) == LitellmUserRoles.PROXY_ADMIN


def test_get_all_team_ids_preserves_claim_order_and_dedupes():
    """Team selection iterates this result, so it must not be a set whose order changes per worker process."""
    jwt_handler = JWTHandler()
    jwt_handler.litellm_jwtauth = LiteLLM_JWTAuth(team_ids_jwt_field="groups")

    team_ids = JWTAuthManager.get_all_team_ids(jwt_handler, {"groups": ["t-delta", "t-alpha", "t-gamma", "t-alpha"]})

    assert team_ids == ("t-delta", "t-alpha", "t-gamma")


@pytest.mark.asyncio
async def test_auth_builder_offers_claim_teams_in_order_with_the_singular_team_first():
    """
    Several claim teams may all serve the requested model. The first one the IdP listed
    must win on every worker process, not whichever a set happened to yield first.
    """
    jwt_handler = JWTHandler()
    jwt_handler.litellm_jwtauth = LiteLLM_JWTAuth(team_id_jwt_field="primary_team", team_ids_jwt_field="groups")
    user_object = LiteLLM_UserTable(user_id="u_order", user_role=LitellmUserRoles.INTERNAL_USER)
    ...
    with (
        patch.object(
            ...
            "auth_jwt",
            new_callable=AsyncMock,
            return_value={"primary_team": "t-primary", "groups": ["t-beta", "t-alpha", "t-beta"]},
        ),
        patch.object(JWTAuthManager, "check_rbac_role", new_callable=AsyncMock),
        patch.object(jwt_handler, "get_rbac_role", return_value=None),
        patch.object(jwt_handler, "get_scopes", return_value=[]),
    ...
    assert mock_find_team_access.call_args.kwargs["team_ids"] == ("t-primary", "t-beta", "t-alpha")
